/* global chrome */

const alarms = chrome.alarms
const browserAction = chrome.browserAction
const logger = new (require('../../src/SimpleLogger.js'))()

logger.setLevel('trace')

var faviconColors = {
  lime: 'data:image/png;base64,',
  red: 'data:image/png;base64,',
  yellow: 'data:image/png;base64,',
  orange: 'data:image/png;base64,'
}

// Copied from ../../src/MithrilUI.js until we can find a better place to put this abstraction
var getIconImage = function (min) {
  if (min < 2) { return faviconColors.red } else if (min < 5) { return faviconColors.orange } else if (min < 15) { return faviconColors.yellow } else { return faviconColors.lime }
}

var msToMin = function (ms) {
  return ms / 60000
}

var minToMs = function (min) {
  return min * 60000
}

/*
 * Manages alarms, used by the Chrome extension to change the color of the icon depending on the
 * remaining time in class.
 *
 * A note on dev mode compatibility: once an alarm has been set, it will not be unset, meaning that
 * alarms will still run on normal time scale if the first alarm was not initialized with dev mode on.
 * To test this properly, dev mode must be enabled within the background script.
 */
class AlarmManager {
  constructor (bellTimer, cookman) {
    this.bellTimer = bellTimer
    this.cookman = cookman
    this.started = false
        // DEBUG
    this.timeScale = bellTimer.devMode ? bellTimer.timeScale : 1
  }

  start () {
    this.started = true
    var min = msToMin(this.bellTimer.getTimeRemainingNumber())
        // initialize color properly
    browserAction.setIcon({ path: getIconImage(min) })

    this.msToNext = 0
    this.nextColor = 'lime'
    this.setNextAlarm()
    alarms.onAlarm.addListener(alarm => {
      browserAction.setIcon({ path: faviconColors[this.nextColor] })
      this.setNextAlarm()
    })
    logger.log('Alarms started.', 'ALARMS')
  }

  setNextAlarm () {
    var msToEnd = this.bellTimer.getTimeRemainingNumber()
    var min = msToMin(msToEnd)
    if (min < 2) {
      this.msToNext = msToEnd
      this.nextColor = 'lime'
    } else if (min < 5) {
      this.msToNext = msToEnd - minToMs(2)
      this.nextColor = 'red'
    } else if (min < 15) {
      this.msToNext = msToEnd - minToMs(5)
      this.nextColor = 'orange'
    } else {
      this.msToNext = msToEnd - minToMs(15)
      this.nextColor = 'yellow'
    }
    var nextTime = this.msToNext / this.timeScale + Date.now()
    alarms.create('Next alarm', { when: nextTime })

    alarms.getAll(alarms => {
      for (let a of alarms) {
        logger.log(`${a.name} fires at ${new Date(a.scheduledTime)} and will change the color to ${this.nextColor}`, 'ALARMS')
        logger.log(`The current correction is ${this.bellTimer.getCorrection()} ms`, 'ALARMS')
      }
    })
  }
}

module.exports = AlarmManager
